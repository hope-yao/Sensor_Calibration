...
def run(rate):
	...
	# 	train_input[i] /= np.max(data_i)
	# 	train_output_max += [np.max(train_output[i])]
	# 	train_output[i] /= np.max(train_output[i])
	# for i, data_i in enumerate(test_input):
	# 	# normalize with testing data
	# 	test_input_max += [np.max(data_i)]
	# 	test_input[i] /= np.max(data_i)
	# 	test_output_max += [np.max(test_output[i])]
	# 	test_output[i] /= np.max(train_output[i])
	#
	...
	batch_size = 8
	num_time_steps = 3000
	input_pl = tf.placeholder(tf.float32, [None, train_input.shape[1]])
	...
	for eq_i in range(max_epoch):
		...
		for i in range(num_itr):
			...
			loss_l2_val, loss_l1_val, loss_mag_val, _ = sess.run([loss_l2, loss_l1, loss_mag, train_op],
																 feed_dict_train)
		...
		print(eq_i, np.mean(ave_loss_l1_val_train), np.mean(ave_loss_l1_val_test), np.mean(ave_loss_l2_val_train),
			  np.mean(ave_loss_l2_val_test), np.mean(ave_loss_mag_val_train), np.mean(ave_loss_mag_val_test))
